...
class AnswerModule(nn.Module):
    ...
    def forward(self, M, questions, answers, word_embedding):
        '''
        M.size() -> B x 1 x hidden_size
        questions.size() -> B x 1 x hidden_size
        answers.size() -> B x seq_len
        '''
        answers = word_embedding(answers)
        # print(answers.size())

        # B x seq_len x hidden_size
        answers = self.self_attn(answers)
        # print(answers.size())
        answers = answers.unsqueeze(2)

        output = torch.bmm(self.w1(questions), answers) + torch.bmm(self.w2(M), answers)
        # print(output.size())
...
class DMNPlus(nn.Module):
    ...
    def forward(self, contexts, questions, answers):
        ...
        preds = self.answer_module(M, questions, answers, self.word_embedding)
        return preds