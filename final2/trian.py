...
from episodic_memory import DMNPlus, DMNPlus2

class TriAN(nn.Module):
    def __init__(self, args, vocab_size_lst):
        ...
        # self.memory_network = DMNPlus(300)
        self.memory_network = DMNPlus2(2 * args['hidden_size'])
    ...
    def forward(self, p, p_pos, p_ner, p_mask, q, q_pos, q_mask, c, c_mask,
        ...
        # memory = self.memory_network(p_sentences, q, self.embeddings.wordEmbedding)
        memory = self.memory_network(p_sentences, q_attn_out, self.embeddings.wordEmbedding)
        # preactivation = (self.p_c_interact(p_attn_out) + self.q_c_interact(q_attn_out) + self.m_c_interact(memory))*c_attn_out
        preactivation = (self.q_c_interact(q_attn_out) + self.m_c_interact(memory))*c_attn_out
